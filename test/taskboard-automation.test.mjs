...
test("the remote automation prompt keeps taskctl local and delegates work to the SSH project", () => {
  ...
  assert.match(prompt, /同一儲存主機目前可用的精確遠端專案對映/);
  ...
  assert.match(prompt, /目前自動化的專案和主機只能作為未繫結議題的首次目標/);
  ...
  assert.match(prompt, /記錄回應 task 的 version 為 ownedVersion[\s\S]*每次 issue move 都必須顯式傳 --if-version ownedVersion/);
