...
const SANDBOX_LABELS: Record<AiChatSandbox, readonly [string, string]> = {
  ...
  "danger-full-access": ["完全存取權限", "Full access"],
};
...
const SANDBOX_DESCRIPTIONS: Record<AiChatSandbox, readonly [string, string]> = {
  ...
  "danger-full-access": ["不受限制地存取網際網路和您電腦上的任何檔案", "Access the internet and any file on your computer without restrictions"],
};
...
const ACTIVITY_LABELS: Record<string, readonly [string, string]> = {
  ...
  command: ["執行指令", "Run command"],
  command_execution: ["執行指令", "Run command"],
...
function activityDetailSummary(
  ...
  if (typeof event.data?.command === "string" && event.data.command.trim()) return text("檢視指令", "View command");
              ...
              {threads.length > 0 ? threads.map((thread) => (
                ...
                >
                  ...
                  <button
                    ...
                    title={text("刪除本機記錄", "Delete local record")}
            ...
            {loading && !snapshot ? (
              ...
              <div className="ai-chat-empty">
                ...
                <p>{projectId
                  ...
                  : text("進入專案後可以新增對話。", "Open a project to start a new chat.")}</p>
              </div>
            )}
          </div>
          ...
          {dangerConfirmOpen && (
            <div className="ai-chat-confirm-backdrop">
              <div className="ai-chat-confirm" role="alertdialog" aria-modal="true" aria-labelledby="ai-chat-confirm-title">
                <strong id="ai-chat-confirm-title">{text("允許完全存取？", "Allow full access?")}</strong>
                <p>{text(
                  "本次訊息允許 Codex 存取工作區之外的檔案和指令。確認只對本次傳送生效。",
