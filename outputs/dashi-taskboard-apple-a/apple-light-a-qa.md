...
独立 Codex 入口的去敏视觉证据见 [screenshots/customized-codex-sidebar-light.png](screenshots/customized-codex-sidebar-light.png)。结构化 DOM JSON 只由源码仓库中的安全捕获脚本写入被 Git 忽略的 `.artifacts/codex-qa/customized-codex`，不在交付包内；证据不包含 token、任务、线程、用户或工作区内容。
