...
export function buildTaskboardAutomationPrompt(request) {
  ...
  const executionInstructions = remoteProject
    ? [
        `本自動化僅在本機作為任務面板控制器執行；實際開發必須派發到 Codex SSH 遠端專案。匯入專案的基礎 identity 是 projectId=${JSON.stringify(request.codexProjectId)}、hostId=${JSON.stringify(request.codexHostId)}、workspacePath=${JSON.stringify(request.workspacePath)}；同一儲存主機目前可用的精確遠端專案對映是 ${JSON.stringify(remoteProjects)}。不要在目前本機自動化會話修改專案檔案。`,
        ...candidateInstructions,
        "先檢查 issue get 的 projectId、version、status、archivedAt、threadId 和 threadBinding。完整 threadBinding 包含 threadId、codexProjectId、codexProjectKind、codexHostId、workspacePath，且它是該議題後續 send、wait 和狀態寫回的唯一目標；目前自動化的專案和主機只能作為未繫結議題的首次目標，不能替換已有繫結。若存在 threadId 但沒有完整 threadBinding，這是隻能由 UI 開啟的 legacy local 繫結：使用 comment add 說明自動化無法確認專案和主機，再使用首次讀取的 version 作為 --if-version、用 --binding-thread-id 保留原 threadId 將議題移動到 blocked；若衝突立即停止。不得 send、create 或覆蓋該繫結。",
        ...
        "確認允許開始後，只有未繫結且仍為未歸檔 todo 的議題才可在讀取程式碼、下載附件、分析或實施前，由目前本機控制器使用剛讀取的 version 移到 in_progress。已有完整 threadBinding 時，issue move 必須同時傳 --binding-thread-id、--binding-codex-project-id、--binding-codex-project-kind、--binding-codex-host-id、--binding-workspace-path 的儲存值，但在舊會話 send/stale 判斷完成前不得把這個 todo 移到 in_progress；stale 清除步驟按後文顯式使用 --clear-binding-thread。未繫結時必須傳 --clear-binding-thread，避免把本機控制器 CODEX_THREAD_ID 寫成任務繫結。寫入成功後記錄回應 task 的 version 為 ownedVersion、projectId 為 ownedProjectId，並記錄本輪 binding；以後本輪每次 issue move 都必須顯式傳 --if-version ownedVersion，成功後再用回應 version 更新 ownedVersion。不得省略 --if-version 後讓 taskctl 自動讀取最新 version。寫入成功前不得繼續。所有認領、評論和狀態寫入只由目前本機控制器完成，不得要求遠端會話執行 taskctl。",
        ...
        "認領成功後，已有完整 threadBinding 時，只能使用其儲存的 threadId 和 codexHostId 呼叫 Codex send_message_to_thread。send 成功後必須重新 issue get 一次，確認 projectId 未變、未歸檔、status 仍為 todo 且完整 threadBinding 與儲存值完全相同；然後由目前本機控制器使用這次複核返回的最新 version、完整舊 binding 和 --if-version 執行 issue move --status in_progress，傳入 --binding-thread-id、--binding-codex-project-id、--binding-codex-project-kind、--binding-codex-host-id、--binding-workspace-path，並記錄回應 task.version 為 ownedVersion。認領成功後繼續執行後文現有 Codex wait_threads、結果評論和 in_review 寫迴路徑，不得結束本輪；若認領發生 409，立即停止，不得重讀新 version 覆蓋。只有舊會話工具明確返回終態 NOT_FOUND 或 CLOSED 等會話不存在或已關閉結果時，才確認 stale。timeout、network failure、Codex host 暫時不可達或 Taskboard service unavailable 都不是 stale：保留 binding 並結束本輪，不得猜測、clear、create 或搶佔。若任務已是 in_progress、活躍、已歸檔、狀態或 binding 已變化，立即停止，不得在目前自動化目標建立替代會話。只有未繫結議題才使用 Codex create_thread 建立遠端任務，target 必須是 {type:\"project\",projectId:actualTarget.codexProjectId,environment:{type:\"local\"}}，首次 identity 必須使用 actualTarget 的 projectId、kind=\"remote\"、hostId 和 workspacePath。傳送給遠端會話的指令必須包含議題編號、標題、完整描述、全部評論和開發上下文，並說明遠端會話不執行 taskctl，只需完成實作、驗證並返回改動、結果和剩餘風險。",
